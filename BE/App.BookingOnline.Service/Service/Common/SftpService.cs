...
public class SftpService : ISftpService
{
    ...
    public async Task<FtpTransJobHistory> DownloadFileTransc(string remoteFilePath, string localFilePath, string filename, string userId, DateTime dateTrans)
    {
        using var client = new SftpClient(_config.Host, _config.Port == 0 ? 22 : _config.Port,
                        _config.UserName, new[] { _config.PrivateKey });

        var jobHistory = new FtpTransJobHistory();
        jobHistory.FileName = filename;
        jobHistory.FilePath = localFilePath;
        jobHistory.SeabankFilePath = remoteFilePath;
        jobHistory.DateRun = DateTime.Now;
        jobHistory.DateId = dateTrans;
        jobHistory.Status = FtpStatus.download.ToString();
        try
        {
            client.Connect();
            var folderPath = localFilePath.Replace("\\" + filename, "");
            ...
            using var s = File.Create(localFilePath);
            client.ChangeDirectory(@"/seabank");
            //var aa = ListAllFiles("seabank");
            client.DownloadFile(remoteFilePath, s);
        ...
        catch (Exception exception)
        {
            ...
            jobHistory.DescriptionError = exception.Message;
            _logger.LogError(exception, $"Failed in downloading file [{localFilePath}] from seabank/ [{remoteFilePath}]");
        }
        finally
        {
            client.Disconnect();
