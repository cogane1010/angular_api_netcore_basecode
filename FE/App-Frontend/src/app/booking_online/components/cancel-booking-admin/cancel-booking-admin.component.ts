...
import { CancelBookingAdminDialogComponent } from './cancel-booking-admin-dialog/cancel-booking-admin-dialog.component';

@Component({
   selector: 'app-cancel-booking-admin',
   templateUrl: './cancel-booking-admin.component.html',
   styleUrls: ['./cancel-booking-admin.component.scss']
})
export class CancelBookingAdminComponent extends BaseComponent implements OnInit {

  displayedColumns = ['isActive','stt','code', 'clientInfo', 'infoBook', 'orgName','specService','price','status', 'createdDate'];
 ...
 displayCourse(org?: Course): string {
  ...
  getData() {
    this.bookingService.getPagingCancel(this.filter).then(
      res => {
        if (!res?.isSuccess) {
  ...
  cancelBooking() {   
    const dialogRef = this.dialog.open(CancelBookingAdminDialogComponent, {
      width: '30vw',
      height:'25vw',
      data: this.listBooking.filter(x => x.isCancel)
    })
